...
### KNN ----
set.seed(7)
BreastCancer_model_knn <- train(diagnosis ~ ., data = breast_cancer,
                                method = "knn", trControl = train_control)

### SVM ----
set.seed(7)
BreastCancer_model_svm <- train(diagnosis ~ ., data = breast_cancer,
                                method = "svmRadial", trControl = train_control)

### Random Forest ----
set.seed(7)
Breastcancer_model_rf <- train(diagnosis ~ ., data = breast_cancer,
...
results <- resamples(list(LDA = BreastCancer_model_lda,
                          KNN = BreastCancer_model_knn, SVM = BreastCancer_model_svm,
                          RF = Breastcancer_model_rf))
